...
| Cline / Cline CLI | 部分支持 | `~/Documents/Cline/Hooks` + `globalState.json` | ✅ | ✅ | ⚠️ | 已支持 native `cancel` 与 `contextModification`，返回结构也更贴近官方 shape，并会在存在时受管 `.clinerules/hooks`；更大返回面仍未做 |
