...
- 现在已经拿到一条真实认证后 `cline --json --yolo` 样本，说明当前环境已能进入真实执行流，而不是停在认证前置。
- setup/UI 仍保留 `cline auth` 提示，用于降低新环境里把认证问题误判成 hooks 安装失败的概率。
- 现在已经拿到一条真实认证后 `cline --json --yolo` 样本，可用于后续继续校准 hooks 行为和输出字段；当前仍以 hooks 路线为主。
